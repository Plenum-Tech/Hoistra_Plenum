...
def test_every_statement_in_the_module_is_covered():
    # Twelve today. Binding: the registration insert for a file no engine claimed, the
    ...
    # soft-reference discovery, the grouping, the fill-the-survivor update, the re-point
    # template and the delete.
    #
    ...
    assert len(STATEMENTS) == 12, f"found {len(STATEMENTS)}: {[s[:40] for s in STATEMENTS]}"
