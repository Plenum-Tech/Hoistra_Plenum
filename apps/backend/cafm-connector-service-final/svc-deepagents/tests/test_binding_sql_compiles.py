...
def test_every_statement_in_the_module_is_covered():
    # Fourteen today. Binding: the registration insert for a file no engine claimed, the
    # session lookup, the building check, the insert that creates a row the owning engine
    # has not written yet, and the two updates. Collapsing duplicates: the hash stamp, the
    # soft-reference discovery, the two group queries (by content and by name), the
    # fill-the-survivor update, the hash the survivor inherits, the re-point template and the delete.
    #
    # If a statement is added this number goes up; if the extraction breaks it goes down,
    # and either way somebody looks. It has already earned its keep twice, catching each
    # insert the moment it was added.
    assert len(STATEMENTS) == 14, f"found {len(STATEMENTS)}: {[s[:40] for s in STATEMENTS]}"
